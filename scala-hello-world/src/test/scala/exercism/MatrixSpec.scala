...
class MatrixSpec extends FunSuite with Matchers {
  ...
  test("require all columns to be the same size") {
  ...
  test("won't blow up on column index out of bound") {
    Matrix("1 2 3\n"
      + "4 5 6\n"
      + "7 8 9").column(12) should be(Vector())
  }

  test("won't blow up on row index out of bound") {
    Matrix("1 2 3\n"
      + "4 5 6\n"
      + "7 8 9").row(12) should be(Vector())
  }
}
